...
export default function timePicker(selector, opt) {
  ...
  for (let i = 0; i < timeSlot + 1; i++) {
    ...
    items.push(`<li><button className="dropdown-item ${
  ...
  let dropdownTemplate = `
    <ul className="dropdown-menu ${options.class.dropdown}" data-picker-id="${id}" style="max-height:400px;overflow:auto">
        ${itemsMarkups}
    </ul>
    `;
  selector.insertAdjacentHTML("afterend", dropdownTemplate);

  let timeSelector = document.querySelectorAll(
    `.${options.class.dropdownItem}`
  );
