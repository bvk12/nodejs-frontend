...
function Header() {
  ...
  const { user, setUser, auth, setAuth, handleLogout } =
  ...
  useEffect(() => {
    ...
    if (!_.isEmpty(user) && !_.isEmpty(auth)) {
      console.log("USER already logged in. user changed...", auth);
                  ...
                  <Dropdown.Toggle bsPrefix as={CustomDropdownToggle}>
                    <div className="d-inline-flex d-sm-none">
                      <Media shape="circle" size="md">
                        ...
                        />
                        <Image src={user?.profileImage} staticImage thumbnail />
                      </Media>
                    </div>
